<template>
  ...
  <div class="relative h-screen flex">
    <!-- Carousel wrapper -->
    <!-- <div class="overflow-hidden relative h-screen sm:h-64 xl:h-screen 2xl:h-96"> -->
    <div class="overflow-hidden top-0 h-screen sm:h-64 xl:h-screen 2xl:h-96">
      <!-- Item 1 -->
    ...
    <!-- Slider indicators -->
    <!-- <div v-if="!noIndicators" class="flex absolute bottom-5 left-1/2 z-50 space-x-3 -translate-x-1/2"> -->
    <div
      v-if="!noIndicators"
      class="absolute bottom-5 left-1/2 flex -translate-x-1/2 space-x-3 rtl:space-x-reverse z-50"
    >
      <button
        v-for="(picture, index) in pictures"
        :key="index"
        :aria-label="'Slide ' + index"
        :class="index === currentPicture ? 'bg-gray-400' : 'bg-gray-100'"
        aria-current="false"
